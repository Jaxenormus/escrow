...
export default async function handleCookieSelection(
  channel: TextBasedChannel,
  ids: Identities,
  type: TRADE_TYPES.LIMITEDS | TRADE_TYPES.ADOPT_ME
): Promise<string> {
  ...
  const findAccount = () =>
    new Promise<string>((resolve, reject) => {
      ...
      const findCookie = async (): Promise<string | void> => {
        const accounts = await container.db.em.find(Account, {
          type,
          tickets: { $eq: null },
        });
        const cookies = await filter(accounts, async account => {
        ...
        if (!isEmpty(cookies)) {
          return cookies[random(0, cookies.length - 1)].id;
        }
      };
      ...
      findCookie().then(accountId => {
        if (accountId) {
          resolve(accountId);
        } else {
          interval(async (_, stop) => {
            if (stopped) stop();
            const accountId2 = await findCookie();
            if (accountId2) {
              resolve(accountId2);
              stopSearch();
              stop();
